...
SELECT A.NOME
FROM APICOLTORE A
JOIN APIARIO AP ON A.ID_APICOLTORE = AP.ID_APICOLTORE
WHERE AP.REGIONE = 'Toscana';

il numero complessivo di apiari per ciascuna regione
SELECT REGIONE, COUNT(CODICE_APIARIO) AS NUMERO_APIARI
FROM APIARIO
GROUP BY REGIONE;

le quantità di miele prodotto in italia lo scorso anno per ciascuna delle quattro tipologie
SELECT T.NOME_TIPOLOGIA, SUM(P.QUANTITA) AS QUANTITA_TOTALE
FROM PRODUZIONE P
JOIN MIELE M ON P.ID_MIELE = M.ID_MIELE
